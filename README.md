...
**Loop over vectorization.** While I found that a vectorized version of the `compute_windows` did exist I chose to use the loop version based on the fact that I had to learn Pandas for this project, so where possible I tried to rely on techniques that I was more familiar with for the sake of correctness. 
